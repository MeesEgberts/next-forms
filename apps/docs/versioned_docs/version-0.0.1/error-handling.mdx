...
# Error Handling
`next-forms` makes it easy to handle both **field-level** and **form-level** errors with full support for Zod validation responses and custom server-side messages.

## Field Errors
When a form is submitted, all fields are automatically validated against your Zod schema on the server. If validation fails, `next-forms` will return a structured `errors` object containing the error messages per field.
...
## Form Errors
Sometimes validation isn’t tied to a specific field. For example: "Invalid credentials" or "Something went wrong".
