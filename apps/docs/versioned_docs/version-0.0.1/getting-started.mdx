...
**`next-server-actions`** is the proper way to handle forms in **Next.js** — built on native APIs, with full support for **Zod** schema validation and **server actions**. It's lightweight, JavaScript-optional, and developer-first.

:::caution
`next-server-actions` requires **Next.js 15 or higher** with the **App Router** and **Server Actions** enabled.
:::
